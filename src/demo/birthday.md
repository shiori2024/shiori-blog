...
<template>
    <div class="main">
        <bokeh v-for="item in items"></bokeh>
        <h1>Happy Birthday!</h1>
        <h2><strong>{{name}}</strong></h2>
        <span @click="handlerURL" :title=age>🎉</span>
        <div class="candle">
...
export default {
    ...
    methods: {
      handlerURL(){
        // this.$router.push('');
        this.window.open("https://github.com/a1046700338/MyPress/blob/main/src/demo/birthday.md?plain=1",'_blank');
      }
    },
    computed: {
      /* 计算属性，年龄计算方法 */
      age() {
          return this.now - this.birthday
      }
    }
};
</script>
